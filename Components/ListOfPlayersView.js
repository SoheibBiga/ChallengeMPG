...
//--Dev Mode--import listOfPlayers from '../Helpers/ListOfPlayersData.json';
...
export default class ListOfPlayersView extends Component {
  constructor(props) {
    ...
    //--Dev Mode--this.rawListOfPlayers = listOfPlayers;
    this.state = {
      listOfPlayers: [],
      position: '',
    };
  ...
  componentDidMount() {
    //--Dev Mode--this.setState({listOfPlayers: listOfPlayers});
  ...
  _filterByName = (text) => {
    if (text == '') {
      ...
      const result = this.rawListOfPlayers.filter((player) =>
        player.lastname.toLowerCase().startsWith(lowerCaseText),
      );
  ...
  _filterByPosition = (itemValue) => {
    if (itemValue == '') {
      ...
      const result = this.rawListOfPlayers.filter(
        (player) => player.ultraPosition == itemValue,
      );
      this.setState({listOfPlayers: result});
    }
    ...
    return (
      <SafeAreaView style={{flex: 1, paddingTop: 10}}>
        <View style={{alignItems: 'center'}}>
          <Text style={[StyleSheet.h1, {marginBottom: 10}]}>
            Liste des joueurs
          </Text>
        </View>
        ...
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          ...
          <Picker
            ...
            onValueChange={(itemValue, itemIndex) =>
              this._filterByPosition(itemValue)
            }>
