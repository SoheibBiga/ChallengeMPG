...
export default class ListOfPlayersItem extends Component {
  _getDetails = () => {
    ...
    const playerApi =
      'https://api.monpetitgazon.com/stats/' + idPlayer + '?season=2018';
    fetch(playerApi, {method: 'GET'})
      ...
      .catch((error) => {
        ...
        Alert.alert(
          'Erreur',
          "Un problème est survenu lors de l'appel à l'API MPG.",
        );
      });
  };
  ...
  render() {
    ...
    const infos = [
      ...
      ['stats', 'avgRate'],
      ['stats', 'sumGoals'],
      ['stats', 'currentChampionship'],
      ['stats', 'percentageStarter'],
    ];
    return (
      <View>
        <TouchableOpacity
          style={{
            ...
            backgroundColor: index % 2 == 0 ? '#D9E9F1' : '#8FAAD9',
          }}
          ...
          {infos.map((element) => {
            ...
            return <Text key={element[element.length - 1]}>{info}</Text>;
          })}
        </TouchableOpacity>
